...
class ResourcesStats(BedwarsStats):
    def __init__(
        self, hypixel_data: HypixelData, mode: Mode = ModesEnum.OVERALL.value
    ) -> None:
    ...
    def get_per_game(self):
        ...
        diamonds_per_game = rround(
            self.diamonds_collected / (self.games_played or 1), 2
        )
        emeralds_per_game = rround(
            self.emeralds_collected / (self.games_played or 1), 2
        )
    ...
    def get_most_modes(self):
        ...
        for resource in resources:
            for mode in ModesEnum.non_dream_modes():
                if mode.is_real:
                    resources[resource][mode.name] = self._bedwars_data.get(
                        f"{mode.prefix}{resource}_resources_collected_bedwars", 0
                    )
