...
class Ratios:
    ...
    def get_per_game(self):
        ...
        return str(final_kills_per_game), str(beds_broken_per_game), str(kills_per_game),\
               str(final_deaths_per_game), str(beds_lost_per_game), str(deaths_per_game)
    ...
    def get_loss_rate(self):
        losses = self.hypixel_data_bedwars.get(f'{self.mode}losses_bedwars', 0)
        return "0%" if self.games_played == 0 else "100%"\
                    if losses == 0 else f'{round((losses / self.games_played) * 100, 2)}%'
