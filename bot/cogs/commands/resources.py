...
class Resources(commands.Cog):
    ...
    async def resources(self, interaction: discord.Interaction, username: str=None):
        ...
        render_resources(name, uuid, mode="Overall", hypixel_data=hypixel_data, save_dir=interaction.id)
        ...
        render_resources(name, uuid, mode="Solos", hypixel_data=hypixel_data, save_dir=interaction.id)
        render_resources(name, uuid, mode="Doubles", hypixel_data=hypixel_data, save_dir=interaction.id)
        render_resources(name, uuid, mode="Threes", hypixel_data=hypixel_data, save_dir=interaction.id)
        render_resources(name, uuid, mode="Fours", hypixel_data=hypixel_data, save_dir=interaction.id)
        render_resources(name, uuid, mode="4v4", hypixel_data=hypixel_data, save_dir=interaction.id)
